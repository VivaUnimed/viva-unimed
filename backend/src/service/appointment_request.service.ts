...
export class AppointmentRequestService {
  ...
  /**
   * Cria uma solicitação administrativamente.
   *
   * Aqui o patientId é permitido porque
   * é o Admin/Técnico selecionando o paciente.
   */
  async create(
    ...
    /**
     * Caso tenha sido selecionado um médico,
     * verifica se ele atende a especialidade.
     */
    if (data.doctorId) {

      const doctorSpecialityBind =
        await DoctorSpecialityModel.findOne({
          where: {
            userId:
              data.doctorId,
    ...
    /**
     * Impede duplicidade de fila
     * para paciente + especialidade.
     */
    const found =
      await AppointmentRequestModel.findOne({
        where: {
          ...
          status:
            "waiting" satisfies AppointmentRequestStatus,
        },
      });
  ...
  async update(
    ...
    const model =
      await AppointmentRequestModel.findByPk(
        id,
      );
  ...
  async list(
    ...
    const where:
      WhereOptions<IAppointmentRequest> = {};
    ...
    const list =
      await AppointmentRequestModel.findAll({
        where,
